	...
	<body>
		...
		<div id="show-item" class="content scaffold-show" role="main">
			...
			<ol class="property-list item">
				...
				<g:if test="${itemInstance?.order}">
				<li class="fieldcontain">
					<span id="order-label" class="property-label"><g:message code="item.order.label" default="Order" /></span>
					
						<span class="property-value" aria-labelledby="order-label"><g:link controller="subscription" action="show" id="${itemInstance?.order?.id}">${itemInstance?.order?.encodeAsHTML()}</g:link></span>
